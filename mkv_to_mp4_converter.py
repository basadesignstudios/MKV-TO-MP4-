...
class DragAndDropSupport:
    ...
    def _register(self):
        ...
        self._old_proc = ctypes.windll.user32.SetWindowLongPtrW(hwnd, GWL_WNDPROC, self._new_proc)

    def unregister(self):
        if sys.platform != "win32" or not self._old_proc or self.hwnd is None:
            return
        hwnd = wintypes.HWND(self.hwnd)
        ctypes.windll.user32.SetWindowLongPtrW(hwnd, GWL_WNDPROC, self._old_proc)
